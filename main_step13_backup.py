...
def run_engine(runner_name: str, payload: dict = Body(...)):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # fallback: if runner prints JSON to stdout
        try:
            return json.loads(proc.stdout)
        except Exception:
            return {"error": "no output.json and stdout not json", "stdout": proc.stdout, "stderr": proc.stderr, "cmd": cmd}
...
def run_and_save(runner_name: str, payload: dict = Body(...)):
    result = run_engine(runner_name, payload)
